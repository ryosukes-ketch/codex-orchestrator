...
class SqliteProjectRepository(ProjectRepository):
    ...
    def _order_tasks_for_execution(tasks: list[Task]) -> list[Task]:
        ...
        for task in tasks:
            local_deps = {d for d in task.depends_on if d in task_ids}
            inbound_count_by_id[task.id] = len(local_deps)
            for dep_id in local_deps:
                dependents_by_id[dep_id].append(task.id)
        ready = sorted(
            [t.id for t in tasks if inbound_count_by_id[t.id] == 0],
            key=lambda tid: position_by_id[tid],
        )
        ordered: list[str] = []
        while ready:
            current = ready.pop(0)
            ordered.append(current)
            for dep in dependents_by_id[current]:
                inbound_count_by_id[dep] -= 1
                if inbound_count_by_id[dep] == 0:
                    ready.append(dep)
            ready.sort(key=lambda tid: position_by_id[tid])
        if len(ordered) != len(tasks):
            return tasks
        tasks_by_id = {task.id: task for task in tasks}
        return [tasks_by_id[tid] for tid in ordered]
    ...
    def save(self, record: ProjectRecord) -> None:
        ...
        try:
            with connection:
                ...
                cursor.execute("DELETE FROM tasks WHERE project_id = ?", (project.id,))
                cursor.execute("DELETE FROM artifacts WHERE project_id = ?", (project.id,))
                cursor.execute("DELETE FROM reviews WHERE project_id = ?", (project.id,))
                cursor.execute("DELETE FROM checkpoints WHERE project_id = ?", (project.id,))
                cursor.execute("DELETE FROM approvals WHERE project_id = ?", (project.id,))
                cursor.execute("DELETE FROM project_history WHERE project_id = ?", (project.id,))
                ...
                for task in record.tasks:
                    cursor.execute(
                        ...
                        """,
                        (
                            task.id, project.id, task.title, task.department.value,
                            task.status.value, json.dumps(task.depends_on), task.note,
                        ),
                    )
                for artifact in record.artifacts:
                    cursor.execute(
                        """
                        INSERT INTO artifacts (id, project_id, task_id, artifact_type, content)
                        VALUES (?, ?, ?, ?, ?)
                        """,
                        (
                            artifact.id, project.id, artifact.task_id,
                            artifact.artifact_type, json.dumps(artifact.content),
                        ),
                    )
                for review in record.reviews:
                    cursor.execute(
                        """
                        INSERT INTO reviews (id, project_id, task_id, verdict, findings)
                        VALUES (?, ?, ?, ?, ?)
                        """,
                        (
                            review.id, project.id, review.task_id,
                            review.verdict, json.dumps(review.findings),
                        ),
                    )
                for checkpoint in record.checkpoints:
                    cursor.execute(
                        """
                        INSERT INTO checkpoints (id, project_id, name, approved, approver, note)
                        VALUES (?, ?, ?, ?, ?, ?)
                        """,
                        (
                            checkpoint.id, project.id, checkpoint.name,
                            1 if checkpoint.approved else 0, checkpoint.approver, checkpoint.note,
                        ),
                    )
                for approval in record.approvals:
                    cursor.execute(
                        ...
                        """,
                        (
                            approval.id, project.id, approval.action_type.value,
                            approval.status.value, approval.reason,
                            approval.requested_by, approval.decision_note,
                        ),
                    )
                cursor.execute(
                    """
                    INSERT INTO project_history (project_id, events) VALUES (?, ?)
                    """,
                    (
                        project.id,
                        json.dumps({
    ...
    def get(self, project_id: str) -> ProjectRecord | None:
        cursor = self._cursor()
        cursor.execute(
            "SELECT id, status, brief_json, policy_json FROM projects WHERE id = ?",
            (project_id,),
        )
        ...
        cursor.execute(
            ...
            "FROM reviews WHERE project_id = ? ORDER BY id",
            (project_id,),
        )
        review_rows = cursor.fetchall()
        cursor.execute(
            "SELECT id, name, approved, approver, note "
            "FROM checkpoints WHERE project_id = ? ORDER BY id",
            (project_id,),
        )
        checkpoint_rows = cursor.fetchall()
        cursor.execute(
            "SELECT id, action_type, status, reason, requested_by, decision_note "
            "FROM approvals WHERE project_id = ? ORDER BY id",
            (project_id,),
        )
        approval_rows = cursor.fetchall()
        cursor.execute(
            "SELECT events FROM project_history WHERE project_id = ?",
            (project_id,),
        )
        ...
        project = Project(
            ...
            policy=ProjectPolicy.model_validate(
                self._load_json(project_row["policy_json"]) or {}
            ),
        )
        tasks = [
            Task(
        ...
        checkpoints = [
            Checkpoint(
                ...
                approver=row["approver"], note=row["note"],
            )
        ...
        history_payload = (
            self._load_json(history_row["events"]) if history_row else {}
        )
        history: list[str] = []
        ...
        if isinstance(history_payload, dict):
            ...
            events = [
                HistoryEvent.model_validate(raw_event) for raw_event in raw_events
            ]
        elif isinstance(history_payload, list):
            if history_payload and isinstance(history_payload[0], dict):
                events = [
                    HistoryEvent.model_validate(e) for e in history_payload
                ]
            else:
                history = history_payload

        return ProjectRecord(
            project=project, tasks=tasks, artifacts=artifacts,
            reviews=reviews, checkpoints=checkpoints, approvals=approvals,
            history=history or [], events=events,
        )
...
def create_repository_from_env() -> ProjectRepository:
    ...
    if backend == "sqlite":
        db_path = os.getenv("SQLITE_DB_PATH", "data/codex.db").strip()
        if not db_path:
            db_path = "data/codex.db"
        db_dir = os.path.dirname(db_path)
        if db_dir:
            os.makedirs(db_dir, exist_ok=True)
        try:
            repository = SqliteProjectRepository(db_path=db_path)
